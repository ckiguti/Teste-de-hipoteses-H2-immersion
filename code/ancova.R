#' ---
#' title: "ANCOVA test for `fss`~`dfs`+`scenario`*`immersion`"
...
#' 
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
car::Boxplot(`fss` ~ `scenario`*`immersion`, data = dat[["fss"]], id = list(n = Inf))
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "dfs", ANCOVA tests with independent between-subjects variables "scenario" (gamified, non.gamified) and "immersion" (lower, upper) were performed to determine statistically significant difference on the dependent varibles "fss". For the dependent variable "fss", there was not statistically significant effects.
#' 
#' 
#' 
#' 
